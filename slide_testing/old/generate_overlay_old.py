...
with h5py.File(img_archive_dir, "r") as img_hf:
    with h5py.File(predictions_archive_dir, "w") as predictions_hf:
        ...
        for img_i in range(img_n):
            ...
            """
            predictions_v_n = int(
                                    np.ceil((img_h/float(sub_h))+1) * 
                                    np.ceil((img_w/float(sub_w))+1)
                                )
            predictions_h = int(np.ceil((img_h/float(sub_h))+1))
            predictions_w = int(np.ceil((img_w/float(sub_w))+1))
            predictions = np.zeros(shape=(predictions_v_n))
            """
            """
            predictions_v_n = (img_h/sub_h) * (img_w/sub_w)
            ...
            predictions = np.zeros(shape=(predictions_v_n))
            #print img.shape
            #print predictions_h, predictions_w, predictions_v_n
            ...
            for row_i in range(img_divide_factor):
                for col_i in range(img_divide_factor):
                    ...
                    #predictions_v = np.zeros(shape=(len(subs)))
                    """
                    ...
                    for sub_i in xrange(0, len(subs), mb_n):
                        ...
                        For now, we use sub_i+batch.shape[0] so that we catch the edges correctly. 
                            This will quickly be deprecated once I add in correct edge handling, however.

                            I don't know why it has to do +1 to overlay correctly
                        """
                        ...
                        try:
                            ...
                            print ""
                            print prediction_i+1,prediction_i+batch.shape[0]+1
                            print batch.shape
                            print nn_classifier.classify(batch).shape
                        """
                    ...
                    """
                    #Reshape predictions for this sub to matrix of same dimensions as original subs matrix
                    ...
                    print subs.shape
                    print sub_img.shape
                    print prediction_i-(subs_h*subs_w), prediction_i, subs_h*subs_w
                    print sub_img_h//sub_h, sub_img_w//sub_w
                    print predictions[(prediction_i-(subs_h*subs_w)):prediction_i].shape
                    print predictions.shape
                    """
                    """
                    THIS NEEDS TO BE 97x114 for the non-edge images, 97x113 for right edges, 96x114 for bottom edges, and 96x113 for bottom-right
                    """
