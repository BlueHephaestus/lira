"""
This file is in charge of loading a new subsection+overlay image for labelling each time the get_next_overlay_subsection() function is called.
...
def get_next_img_subsection(img_i, sub_i, sub_h, sub_w, factor, img_archive_dir):
    """
...
def get_next_overlay_subsection(
        img_i,
        sub_i,
        alpha=1/3.,
        sub_h=80, 
    ...
    """
    Our main function in this file.

    Given our image index and subsection index, as well as metadata for information and directories,
        ...
        with h5py.File(predictions_archive_dir, "r") as predictions_hf:
            ...
            for img_i in range(img_n):
                ...
                for sub_i in range(sub_n):
                    ...
                    greyscale_sub = get_next_img_subsection(img_i, sub_i, sub_h, sub_w, factor, img_archive_dir)
...
We are making this it's own standalone file, so that the main function is given an img_i and sub_i, so that it doesn't loop through everything, it just gets one at a time, depending on where our main slide_handler is.
"""
